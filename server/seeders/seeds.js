...
db.once("open", async () => {
  // try {
    ...
    for (let i = 0; i < 50; i += 1) {
      const activity = chance.pickone(["row", "kayak", "swim", "sup", "bike"]);

      const randomUserIndex = Math.floor(Math.random() * createdUsers.ops.length);
      const { username, _id: userId } = createdUsers.ops[randomUserIndex];

      const meters = chance.integer({ min: 1, max: 10000 });
      const date = chance
      .date({ year: 2023 })
      .toISOString()
      .substring(0, 10)
      ...
      const createdWorkout = await Workout.create({ activity, meters, username, date, notes})
